...
public interface DisplayNameGenerator {
	...
	class IndicativeSentences implements DisplayNameGenerator {
		 ...
		 *
		 * <p>Note that {@code @SentenceFragment} always takes precedence over
		...
		public String generateDisplayNameForMethod(List<Class<?>> enclosingInstanceTypes, Class<?> testClass,
			...
			Optional<String> sentenceFragment = getSentenceFragment(testMethod);
			displayName += (sentenceFragment.isPresent() ? sentenceFragment.get()
		...
		private String getSentenceBeginning(Class<?> testClass, List<Class<?>> enclosingInstanceTypes) {
			...
			Optional<String> displayName = getSentenceFragment(testClass);
			if (!displayName.isPresent()) {
				displayName = findAnnotation(testClass, DisplayName.class)//
		...
		private static final Optional<String> getSentenceFragment(AnnotatedElement element) {
			return findAnnotation(element, SentenceFragment.class).map(SentenceFragment::value).map(String::trim);
		}
